...
@section('content')
        ...
        <div class="overflow-x-auto">
                            ...
                            <td class="px-4 py-3 text-center space-x-3">
                                <div class="flex gap-3 items-center justify-center">
                                    @if (Auth::guard('company')->check() == true || Auth::user()->can('view transaction'))
                                        <a href="{{ route('transaction.show', $transaction) }}">
                                            <div class="relative group">
                                                ...
                                                <span class="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block bg-gray-800 text-white text-xs rounded-md px-2 py-1 z-[99999] w-max max-w-[110px] text-center leading-tight">
                                    ...
                                    @if (Auth::guard('company')->check() == true || Auth::user()->can('delete transaction'))
                                        <div class="relative group">
                                            <button
                                                onclick="confirmDeleteTransaction(event, '{{ route('transaction.destroy', $transaction->id) }}')"
            ...
            <div class="text-center py-12">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">
                    @if (request('search'))
                        No transaction found for "{{ request('search') }}"
                    @else
    ...
    <script>
        function confirmDeleteTransaction(event, deleteUrl) {
