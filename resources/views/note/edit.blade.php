...
@section('content')
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-lg mt-6">
        ...
        <form action="{{ route('note.update', $note->id) }}" method="POST" id="note">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" placeholder="description"
                    class="w-full px-4 py-2 border border-gray-300 rounded-md placeholder:capitalize">{{ old('description', $note->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            {{--         id="voucher_code"> --}}
            {{--         <option disabled>Select Voucher Code</option> --}}
            {{--         @foreach ($transactions as $transaction) --}}
            {{--             <option value="{{ $transaction->voucher_code }}" --}}
            {{--                 {{ (old('voucher_code', $note->voucher_code) == $transaction->voucher_code) ? 'selected' : '' }}> --}}
    ...
    <script>
        ...
        function addNoteRow(data) {
            ...
            const newId = Math.max(...noteDetails.map(item => item.id), 0) + 1;
            ...
            row.innerHTML = `
        <td class="px-4 py-3 text-center">${tbody.rows.length}</td>
        ...
        <td class="px-4 py-3 text-center">
            <button class="edit-btn text-blue-500 hover:underline" data-id="${newId}">Edit</button> |
            <button class="delete-btn text-red-500 hover:underline" data-id="${newId}">Delete</button>
        </td>
    `;
