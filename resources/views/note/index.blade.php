  ...
  {{--   @endif --}}
    ...
    <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-lg mt-6">
        ...
        <div class="flex justify-between mb-6 items-center">
            ...
            <!-- Search and Sorting -->
            <form method="GET" action="{{ route('note.index') }}" class="flex gap-2 flex-wrap">
            ...
            <div class="flex gap-3">
                @if (Auth::guard('company')->check() || Auth::user()->can('create note'))
                ...
                {{-- @if (Auth::guard('company')->check() || Auth::user()->can('compare note')) --}}
                    ...
                    <tbody class="text-center">
                        @foreach ($notes as $index => $note)
                            <tr class="border-gray-200 border-b hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3 w-64 truncate overflow-hidden whitespace-nowrap max-w-xs">{{ $note->description}}</td>
                                ...
                                {{-- </td> --}}
                                <td>Rp {{ number_format($note->total_price, 0, ',', '.') }}</td> <!-- Menampilkan grand_total -->
                                ...
                                <td class="px-4 py-3 text-center space-x-3">
                                    <div class="flex gap-3 items-center justify-center">
                                        @if (Auth::guard('company')->check() == true || Auth::user()->can('view note'))
                                            <a href="{{ route('note.show', $note) }}">
                                                <div class="relative group">
                                                    ...
                                                    <span class="capitalize absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block bg-gray-800 text-white text-xs rounded-md px-2 py-1">
                                                        View note Detail
                                                    </span>
                                                </div>
                                            </a>
                                        ...
                                        @if (Auth::guard('company')->check() == true || Auth::user()->can('update note'))
                                        ...
                                        @if (Auth::guard('company')->check() == true || Auth::user()->can('compare note'))
                                            <a href="{{ route('note.compare', $note->id) }}">
                                                <div class="relative group">
                                                    <div class="p-[5px] bg-cyan-50 rounded-md">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 text-cyan-500">
                                        ...
                                        @if (Auth::guard('company')->check() == true || Auth::user()->can('delete note'))
                                            <div class="relative group">
                                                <button
